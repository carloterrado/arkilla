...
<header class="flex items-center h-20 uppercase bg-teal-500 px-9 font-semibold shadow-md">
    ...
    <nav class="{showNavBar === true ? 'translate-x-0' : '-translate-x-full'} mx-auto fixed left-0 top-0 sm:translate-x-0 transition duration-1000 bg-slate-600 w-full h-screen p-6 sm:p-0 sm:h-auto sm:relative sm:grid sm:place-items-center sm:bg-transparent">
        ...
        <ul class="flex flex-col sm:flex-row gap-4 sm:gap-8">
            <li><a href="/" class="uppercase relative before:absolute before:left-0 before:-bottom-[2px] before:content-[''] before:h-[2px] before:w-full before:bg-black before:scale-x-0 before:origin-right hover:before:scale-x-100 hover:before:origin-left before:transition-transform before:duration-500" on:click="{() => showNavBar = false}">Home</a></li>
            <li><a href="/product" class="uppercase relative before:absolute before:left-0 before:-bottom-[2px] before:content-[''] before:h-[2px] before:w-full before:bg-black before:scale-x-0 before:origin-right hover:before:scale-x-100 hover:before:origin-left before:transition-transform before:duration-500" on:click="{() => showNavBar = false}">Product</a></li>
            <li><a href="/about" class="uppercase relative before:absolute before:left-0 before:-bottom-[2px] before:content-[''] before:h-[2px] before:w-full before:bg-black before:scale-x-0 before:origin-right hover:before:scale-x-100 hover:before:origin-left before:transition-transform before:duration-500" on:click="{() => showNavBar = false}">About</a></li>
            <li><a href="/services" class="uppercase relative before:absolute before:left-0 before:-bottom-[2px] before:content-[''] before:h-[2px] before:w-full before:bg-black before:scale-x-0 before:origin-right hover:before:scale-x-100 hover:before:origin-left before:transition-transform before:duration-500" on:click="{() => showNavBar = false}">Services</a></li>
        </ul>
    </nav>
    {#if $user != null}
